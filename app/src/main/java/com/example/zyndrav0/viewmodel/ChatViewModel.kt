...
class ChatViewModel(
    ...
    private val gachaRepository = com.example.zyndrav0.data.repository.GachaRepository(
    ...
    private var _currentConversationId = conversationId

    var statusText by mutableStateOf("Estado: Listo para enviar.")
        private set

    init {
        if (_currentConversationId != -1L) {
            loadMessages()
        }
    ...
    private fun loadMessages() {
        viewModelScope.launch {
            chatRepository.getMessagesByConversation(_currentConversationId).collect { _messages.value = it }
        }
    }
    ...
    fun sendMessage(messageText: String, attachmentUri: Uri? = null, attachmentType: String? = null) {
        ...
        viewModelScope.launch {
            ...
            if (_currentConversationId == -1L) {
                _currentConversationId = chatRepository.createConversation(userId, "Nuevo Chat")
            ...
            try {
                statusText = "Enviando..."
                val apiResponse = RetrofitClient.instance.sendMessage(
                    ChatMessage(
                        sender = "user", 
                        message = messageText, 
                ...
                chatRepository.insertMessage(
                    conversationId = _currentConversationId,
                    text = apiResponse, // La respuesta ahora es un String (tuvimos problemas con JSON)
                    isUser = false
                )
                statusText = "Listo"
